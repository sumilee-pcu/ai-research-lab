...
"AI에게 분석 코드를 시키면 어디까지 맡기고 어디서 검증해야 하는가"를 실제 코드로 보여줍니다. 두 스크립트 모두 **고정 시드(seed=42)로 가상 데이터를 생성**하므로, 누가 실행해도 **똑같은 결과**가 나옵니다(재현성). 실제 연구에서는 이 가상 데이터 부분을 본인의 데이터 파일 로드로 바꾸면 됩니다.
